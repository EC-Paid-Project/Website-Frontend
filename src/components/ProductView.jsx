...
const ProductView = (props) => {
  const renderBanner = () => {
    if (props.discount) {
      return (
        <div className="product-view-banner bg-red-500 text-yellow-200 font-bold py-1 px-2 absolute  text-center top-0 right-0 w-1/8 rounded">
          On Sale
        </div>
      );
    }
      ...
      <div className={`product-view-card ${props.color1} relative`}>
        ...
        <div className="product-view-section p-4">
          <Link to={`/product/${props.id}`}>
            <h3 className="product-view-title  font-bold mb-2">
              {props.title}
            </h3>
          </Link>
          {props.discount ? (
            <div className="product-view-price-wrapper mb-2">
              <span className="product-view-cancel-price line-through ">
                PKR {props.price}
              </span>
          ...
          <Link to={`product/${props.id}`}>
            <div className="product-view-button     text-white rounded-md hover:bg-blue cursor-pointer">
              Shop Now
            </div>
          </Link>
        </div>
